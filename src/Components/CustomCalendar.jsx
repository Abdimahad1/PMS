...
const CustomCalendar = () => {
    ...
    return (
        <div className={`calendar-container ${theme.background} p-4 h-screen overflow-hidden`}>
            <h1 className={`text-4xl font-bold mb-4 text-center ${theme.text}`}>2024 CALENDAR FOR OUR PROJECT MANAGEMENT SYSTEM</h1>
                ...
                {Array.from({ length: 12 }, (_, i) => (
                    <div key={i} className="month mb-4 p-2">
                        <Calendar
                            ...
                            tileClassName={({ date, view }) => (view === 'month' && date.getDay() === 0 ? 'text-red-500' : '')}
                        />
                    </div>
                ))}
            </div>
